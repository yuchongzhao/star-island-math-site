...
返回 {"readable":true,"reason":"","pages":[{"image":1,"lesson":"照片上能确认的篇名；没有则写课文片段","page":null,"text":"逐字识别正文，保留标点、换行。不要抄注释、拼音音节、侧栏、问题和解说。只收录看清楚且有连续完整句子的内容","assignment":"照片中明确的背诵要求；无则空字符串"}]}。
...
def validate_pages(result, count):
    ...
    for p in pages:
        ...
        seen.add(image)
        clean.append({'image':image,'page':number,'lesson':string(p.get('lesson'),80),'text':string(p.get('text'),6000),'assignment':string(p.get('assignment',''),300,False)})
...
选择 6—12 个有记忆价值的字词（短诗可少于 6 个），每个 2—8 字，带准确语境读音。背默任务分成 2—8 小段，每段 1—2 完整句、最多 60 个汉字，保持课文次序；只在原文足够时生成，不拼接不连续句子。优先照片明确要求背诵的部分，无明确要求时仅作为练习建议，不声称老师指定。
